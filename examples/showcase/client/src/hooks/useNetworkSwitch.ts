...
export function useNetworkSwitch() {
   ...
   */
  const switchToNetwork = useCallback(async (network: Network): Promise<boolean> => {
    if (!connector) {
          ...
          try {
            await provider.request({
              ...
              params: [{
                ...
                blockExplorerUrls: config.chain.blockExplorers ? [config.chain.blockExplorers.default.url] : undefined,
              }],
            });
